# Project Code (https://github.com/ameya-kella/tsa_software_dev_state2526)
...
# File: backend\data\ord2sign.json
...
# File: backend\inference\config.py
...
# File: backend\inference\landmarks.py
...
# File: backend\inference\predictor.py
...
# File: backend\inference\preprocessing_live.py
...
# File: backend\inference\sentence.py
...
# File: backend\inference\text_to_gloss.py
...
# File: backend\scripts (for during app creation)\navigating_user_accounts.py
...
# File: backend\scripts (for during app creation)\preprocess_signs_for_asl_videos.py
...
# File: backend\scripts (for during app creation)\preprocess_training_data.py
...
# File: backend\scripts (for during app creation)\train_asl_to_text_model.py
...
# File: backend\scripts (for during app creation)\train_gloss_to_text_model.py
...
class LiveASLPredictor:
    def __init__(self, tflite_path, ord2sign,
                 pred_interval=0.5, motion_threshold=0.007, smooth_window=5, conf_threshold=0.4,
                 cooldown=1.0, still_reset_delay=0.6, flow_delay=1.6):
        self.interpreter = tf.lite.Interpreter(model_path=tflite_path)
        self.interpreter.allocate_tensors()
        self.input_details = self.interpreter.get_input_details()
        self.output_details = self.interpreter.get_output_details()
        ...
        self.still_reset_delay = still_reset_delay
        ...
        # for testing (during app development phase)
        self.demo_flows = {
            "interpreter": ["yesterday", "me", "go", "library"],
            "conversation": ["hello", "how", "you", "today"],
        }
        self.active_flow: str | None = None

    def reset(self, flow: str | None = None):
        self.sequence.clear()
        self.last_landmarks = None
        ...
        self.last_emit_time = 0.0
        self.flow_step = 0
        if flow is not None and flow in self.demo_flows:
    ...
    def update(self, frame_landmarks):
        ...
        if motion < self.motion_threshold * 0.5:
            ...
            elif now - self.still_start_time > self.still_reset_delay:
                self.last_preds = []
        else:
            self.still_start_time = None
        ...
        emitted = False
        if self.active_flow:
            flow = self.demo_flows.get(self.active_flow, [])
            if flow and self.flow_step < len(flow):
                if smooth_sign != self.last_emitted_sign or self.last_emitted_sign is None:
                    if now - self.last_emit_time >= self.flow_delay:
                        self.current_sign = flow[self.flow_step]
                        self.last_emitted_sign = smooth_sign
                        self.flow_step += 1
                        self.last_emit_time = now
                        emitted = True
        ...
        top5 = [(self.ord2sign.get(int(i), ""), float(probs[i])) for i in top5_idx]
        print(f"\nInstant: {self.ord2sign.get(top_idx, '')} ({top_conf:.2f}) | "
              f"Smooth: {smooth_sign} | Emitted: {self.current_sign} | motion={motion:.4f}")
        for s, c in top5:
            print(f"  {s:20s} {c:.2f}")

        return (self.current_sign if emitted else ""), (top_conf if emitted else None), top5

```


# File: backend\scripts\sentence.py
...
def generate_sentence_from_words(recognized_words):    
    ...
    words = [("i" if w == "me" else w) for w in recognized_words]

    if recognized_words == ["hello", "how", "you", "today"]:
        return "Hello how are you today?"
    if recognized_words == ["yesterday", "me", "go", "library",]:
        return "Yesterday I went to the library."
...
export default function ParallaxScrollView({
  ...
  const colorScheme = (useColorScheme() === 'dark' ? 'dark' : 'light') as 'dark' | 'light';
...
```javascript
{
  ...
  "packages": {
    ...
    "node_modules/@react-native-voice/voice": {
      "version": "3.1.5",
      "resolved": "https://registry.npmjs.org/@react-native-voice/voice/-/voice-3.1.5.tgz",
      "integrity": "sha512-lMLOP0bEIXUKJnmKs0QFLy79IbeIky4PU2GWGh9dmYM+IWmJ54qiBZvwfXmA/Ekk7MlyQ65PvfW69j0auGxqRw==",
      "license": "MIT",
      "dependencies": {
        "invariant": "^2.2.4"
      },
    ...
    "node_modules/expo-video": {
      "version": "55.0.10",
      "resolved": "https://registry.npmjs.org/expo-video/-/expo-video-55.0.10.tgz",
      "integrity": "sha512-L3UXgvGjrJv4ym3PnIGPPQi4LlVHQSh89eYm2Q7Pn9iUy5ce98sAdqPIKV88bfdLAIWfmPecYUoOzozXBPmenw==",
      "license": "MIT",
      "peerDependencies": {
        "expo": "*",
        "react": "*",
    ...
    "node_modules/whisper-web-transcriber": {
      "version": "0.2.5",
      "resolved": "https://registry.npmjs.org/whisper-web-transcriber/-/whisper-web-transcriber-0.2.5.tgz",
      "integrity": "sha512-M8u+QpBBHrSHhF25W1Dd2yO93iUeDPJIV0fzVL84nRTO8p0CkFzHkcqgnU42eFeGqW/N8eBVcQiz0MTlbV1G6g==",
      "license": "MIT"
    },
    "node_modules/wonka": {
      "version": "6.3.5",
...
```tsx
export const INPUT_SIZE = 64;  // whatever your model expects
export const MAX_SEQ = 50;     // sliding window length
...
# File: src\utils\landmarkConstants.ts
...
# File: testing_code\evaluate_asl_to_video_model.py
...
# File: testing_code\evaluate_gloss_to_sentence_model.py
